...
function ProposalForm() {
  ...
  const onClickHandler = (e) => {
    e.preventDefault();
    validation(formValues).then(() => submit(formValues,dispatch)).catch((error)=>{
      dispatch(detailsActions.toggleModalOpen()); 
      dispatch(detailsActions.setSubmitState({
  ...
  return (
    <div className="proposal-form flex flex-col px-2 py-2 ">
      <h1 className="title pb-4 text-lg font-semibold text-theme-blue m-auto">
        Submit an answer!
      </h1>
      <form>
        <TextInput />
      ...
      {modalState &&
          <ModalT open={modalState}>
            <ModalContent submitStatus={submitStatus} close={onClickHandlerModal}/>
          </ModalT>
      }
