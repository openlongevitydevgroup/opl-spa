...
function StatbarButtonGroup(){
    ...
    return(
        ...
        <TooltipWrapper message='View questions as flat list'>
          <ToggleButton value="table" onClick={toggleTableHandler} className={`${isMobileState ? 'w-1/3' : null}`}>
            <TableViewIcon />
          </ToggleButton>
        </TooltipWrapper>
        <TooltipWrapper message="View questions as hierarchical list">
        <ToggleButton value="tree" onClick={toggleTreeHandler} className={`${isMobileState ? 'w-1/3' : null}`}>
          <AccountTreeIcon />
        </ToggleButton>
        </TooltipWrapper>
